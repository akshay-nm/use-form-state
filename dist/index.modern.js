...
const useFormState = (spec, debug = false) => {
  ...
  const [values, setValues] = useState(spec.states.map(state => state.default));
  ...
  const [valids, setValids] = useState(spec.states.map(state => state.isValid));
  const [needValidations, setNeedValidations] = useState(spec.states.map(() => false));
  ...
  const [onChangeHandlers] = useState(spec.states.map((state, index) => value => onChangeHandler(index, value)));
  const [resetStates] = useState(spec.states.map((state, index) => () => resetState(index)));
  const [setStates] = useState(spec.states.map((state, index) => (value, isValid, needsValidation) => setState(index, value, isValid, needsValidation)));
  useEffect(() => {
