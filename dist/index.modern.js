...
const generateReturnPayload = (names = [], values = [], warnings = [], onChangeHandlers = [], valids = []) => {
  const payload = {};
...
const useFormState = ({
  ...
  const [needValidations, setNeedValidations] = useState(states.map(() => false));
  ...
  const [onChangeHandlers] = useState(states.map((state, index) => value => onChangeHandler(index, value)));
  const [resetStates] = useState(states.map((state, index) => () => resetState(index)));
  const [setStates] = useState(states.map((state, index) => (value, isValid, needsValidation) => setState(index, value, isValid, needsValidation)));
  useEffect(() => {
